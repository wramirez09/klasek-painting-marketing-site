...
const Footer = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<footer class="flex flex-col items-center bg-secondary px-6">${validate_component(EmailSignup, "EmailSignup").$$render($$result, {}, {}, {})} ${validate_component(FooterCta, "FooterCta").$$render($$result, {}, {}, {})} <div class="w-full py-8 p-x grid lg:grid-cols-4 grid-cols-2 lg:gap-x-12 gap-y-8 xs:gap-x-4"> <div class="flex flex-col gap-6 lg:col-span-1 col-span-2 max-w-[200px] mx-auto"><a href="/">${typeof logo === "string" ? `<img${add_attribute("src", logo.img.src, 0)} alt="Klasek Painting" class="w-full h-auto" loading="lazy"${add_attribute("width", logo.img.w, 0)}${add_attribute("height", logo.img.h, 0)}>` : `<picture>${each(Object.entries(logo.sources), ([format, srcset]) => {
    return `<source${add_attribute("srcset", srcset, 0)}${add_attribute("type", "image/" + format, 0)}>`;
