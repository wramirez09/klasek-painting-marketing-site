import { c as create_ssr_component, o as onDestroy, b as escape, a as add_attribute, e as each, g as subscribe, v as validate_component } from "../../../../../../chunks/ssr.js";
...
import { r as routes, s as serviceAreaRoutes } from "../../../../../../chunks/routes.js";
...
import { s as splash } from "../../../../../../chunks/orange-paint-splash.js";
import { s as stars } from "../../../../../../chunks/5-stars.js";
import { g as google } from "../../../../../../chunks/Google-name-logo.js";
...
const phone = "(708) 267-0682";
const phoneHref = "tel:+17082670682";
const EstimateFormBand = create_ssr_component(($$result, $$props, $$bindings, slots) => {
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return ` <div class="bg-white border-b text-center text-[13px] font-bold text-gray-700 py-2 p-x" data-svelte-h="svelte-1s8jnrs"><span class="inline-flex items-center gap-2"><span class="w-2 h-2 rounded-full bg-primary animate-pulse" aria-hidden="true"></span>
    #1 Hardie Board Painting Company in Cook County — Book Your Free Estimate</span></div>  <section class="relative bg-secondary-dark text-white overflow-hidden"><div class="absolute inset-0 bg-gradient-to-br from-secondary/60 via-secondary-dark to-secondary-dark" aria-hidden="true"></div> <div class="absolute -top-24 -right-24 w-[480px] h-[480px] rounded-full bg-primary/10 blur-3xl" aria-hidden="true"></div> <div class="container relative grid lg:grid-cols-[440px_1fr] gap-10 lg:gap-14 items-center py-14 lg:py-20 p-x"> <div class="order-2 lg:order-1"><div class="relative">${typeof heroSrc === "string" ? `<img class="w-full h-auto aspect-[4/3] object-cover ring-4 ring-white/10 rounded-xl shadow-2xl"${add_attribute("src", heroSrc.img.src, 0)} alt="Suburban home with fiber-cement lap siding and board-and-batten gables"${add_attribute("loading", $isMobileStore ? "lazy" : "eager", 0)}${add_attribute("width", heroSrc.img.w, 0)}${add_attribute("height", heroSrc.img.h, 0)}>` : `<picture>${each(Object.entries(heroSrc.sources), ([format, srcset]) => {
    return `<source${add_attribute("srcset", srcset, 0)}${add_attribute("type", "image/" + format, 0)}>`;
  })} <img class="w-full h-auto aspect-[4/3] object-cover ring-4 ring-white/10 rounded-xl shadow-2xl"${add_attribute("src", heroSrc.img.src, 0)} alt="Suburban home with fiber-cement lap siding and board-and-batten gables"${add_attribute("loading", $isMobileStore ? "lazy" : "eager", 0)}${add_attribute("width", heroSrc.img.w, 0)}${add_attribute("height", heroSrc.img.h, 0)}></picture>`} ${validate_component(GoogleProof, "GoogleProof").$$render(
    $$result,
    {
      class: "bg-white rounded-xl border p-4 sm:absolute sm:-bottom-8 sm:-right-4 mt-4 sm:mt-0 w-full sm:w-[340px] shadow-2xl"
    },
    ...
    return `<source${add_attribute("srcset", srcset, 0)}${add_attribute("type", "image/" + format, 0)}>`;
        ...
        siding has deteriorated — often where it meets the ground — we repair or
        replace the damaged section so the finish goes on solid and lasts.</p> <div class="mt-7 grid sm:grid-cols-2 gap-x-6 gap-y-4">${each(repairItems, ([title, body]) => {
    return `<div class="flex items-start gap-3"><span class="mt-1.5 w-2 h-2 rounded-full bg-primary-dark shrink-0" aria-hidden="true"></span> <p><b class="font-bold text-secondary-dark">${escape(title)}</b><br> <span class="text-gray-600 text-[15px]">${escape(body)}</span></p> </div>`;
    ...
    return `<source${add_attribute("srcset", srcset, 0)}${add_attribute("type", "image/" + format, 0)}>`;
  })} <img class="absolute bottom-0 right-0 w-[60%] aspect-[4/3] object-cover rounded-lg ring-[6px] ring-primary-dark shadow-2xl"${add_attribute("src", guideAngle.img.src, 0)} alt="Classic two-story home with fiber-cement siding and balcony" loading="lazy"${add_attribute("width", guideAngle.img.w, 0)}${add_attribute("height", guideAngle.img.h, 0)}></picture>`}</div></div>  <div class="mt-14 grid md:grid-cols-3 gap-6">${each(guideCards, ({ number, label, title, body }) => {
    return `<div class="rounded-2xl border bg-off-white p-7"><span class="inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.12em] text-primary-dark"><span class="w-6 h-6 rounded-full bg-primary-dark text-white flex items-center justify-center text-[13px]">${escape(number)}</span> ${escape(label)}</span> <h3 class="mt-4 font-bold text-xl text-secondary-dark">${escape(title)}</h3> <p class="mt-3 text-gray-600 leading-relaxed">${escape(body)}</p> </div>`;
  ...
  )}</div></div></section>  <section id="why" class="bg-white p-y p-x scroll-mt-20"><div class="container"><div class="text-center max-w-2xl mx-auto mb-11" data-svelte-h="svelte-1fia0mq"><div class="inline-block bg-primary-dark text-white px-7 py-2.5 -skew-x-6 shadow-subtle mb-5"><span class="inline-block skew-x-6 font-extrabold text-lg sm:text-xl uppercase tracking-wide">Why Choose Klasek</span></div> <h2 class="text-3xl sm:text-4xl leading-[1.08] font-extrabold text-secondary-dark">Three decades on Chicagoland ladders. One crew, start to finish.</h2></div> <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">${each(stats, ({ stat, title, body }) => {
    return `<div class="rounded-xl border p-6 text-center hover:shadow-subtle transition"><div class="font-extrabold text-4xl text-primary-dark">${escape(stat)}</div> <h3 class="mt-2 font-bold text-lg text-secondary-dark">${escape(title)}</h3> <p class="mt-2 text-gray-600 text-[15px] leading-relaxed">${escape(body)}</p> </div>`;
  })}</div></div></section>  <section class="bg-off-white p-y p-x"><div class="container"><div class="text-center mb-9 flex flex-col items-center" data-svelte-h="svelte-oo9x63"><span class="tab mb-4 svelte-1gd76v4">Our work</span> <h2 class="text-3xl sm:text-4xl leading-[1.08] font-extrabold text-secondary-dark">Before &amp; after, around the neighborhood.</h2></div> <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">${each(galleryImages, ({ src, alt }) => {
    return `${typeof src === "string" ? `<img class="w-full h-full aspect-square object-cover rounded-lg"${add_attribute("src", src.img.src, 0)}${add_attribute("alt", alt, 0)} loading="lazy"${add_attribute("width", src.img.w, 0)}${add_attribute("height", src.img.h, 0)}>` : `<picture>${each(Object.entries(src.sources), ([format, srcset]) => {
      return `<source${add_attribute("srcset", srcset, 0)}${add_attribute("type", "image/" + format, 0)}>`;
    })} <img class="w-full h-full aspect-square object-cover rounded-lg"${add_attribute("src", src.img.src, 0)}${add_attribute("alt", alt, 0)} loading="lazy"${add_attribute("width", src.img.w, 0)}${add_attribute("height", src.img.h, 0)}> </picture>`}`;
  ...
  )}</div></div> <div><ul class="grid grid-cols-2 sm:grid-cols-3 gap-x-5 gap-y-2.5 text-[15px] font-semibold text-gray-700">${each(Object.values(serviceAreaRoutes), ({ text, href }) => {
    return `<li class="flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-primary-dark shrink-0" aria-hidden="true"></span> <a class="hover:text-primary-dark hover:underline"${add_attribute("href", href, 0)}>${escape(text)}</a> </li>`;
  })}</ul> <div class="mt-6 h-44 rounded-lg overflow-hidden">${validate_component(Map, "Map").$$render($$result, { lazy: true }, {}, {})}</div></div></div></div></section>  <section class="bg-off-white py-14 border-y p-x"><div class="container"><div class="mb-8" data-svelte-h="svelte-1jx66ah"><div class="ribbon svelte-1gd76v4"><span class="flag flag-l svelte-1gd76v4" aria-hidden="true"></span> <p class="font-bold text-lg text-secondary-dark uppercase tracking-wide text-center">Our Trusted Brand Partners</p> <span class="flag flag-r svelte-1gd76v4" aria-hidden="true"></span></div></div> <div class="flex flex-wrap items-center justify-center gap-x-10 sm:gap-x-14 gap-y-7">${each(partnerLogos, ({ src, alt, height }) => {
    return `${typeof src === "string" ? `<img class="${escape(height, true) + " w-auto object-contain opacity-75 hover:opacity-100 transition svelte-1gd76v4"}"${add_attribute("src", src.img.src, 0)}${add_attribute("alt", alt, 0)} loading="lazy"${add_attribute("width", src.img.w, 0)}${add_attribute("height", src.img.h, 0)}>` : `<picture>${each(Object.entries(src.sources), ([format, srcset]) => {
      return `<source${add_attribute("srcset", srcset, 0)}${add_attribute("type", "image/" + format, 0)}>`;
    })} <img class="${escape(height, true) + " w-auto object-contain opacity-75 hover:opacity-100 transition svelte-1gd76v4"}"${add_attribute("src", src.img.src, 0)}${add_attribute("alt", alt, 0)} loading="lazy"${add_attribute("width", src.img.w, 0)}${add_attribute("height", src.img.h, 0)}> </picture>`}`;
  })}</div></div></section>  <section id="faq" class="bg-white p-y p-x scroll-mt-20"><div class="container max-w-3xl"><div class="mb-10" data-svelte-h="svelte-1d4vlt2"><div class="ribbon svelte-1gd76v4"><span class="flag flag-l svelte-1gd76v4" aria-hidden="true"></span> <h2 class="text-3xl sm:text-4xl leading-[1.08] font-extrabold text-secondary-dark text-center">Frequently Asked Questions</h2> <span class="flag flag-r svelte-1gd76v4" aria-hidden="true"></span></div></div> <div class="divide-y border-y">${each(faqData, ({ question, answer }, i) => {
    return `<div class="py-5"><button type="button" class="flex w-full items-center justify-between gap-4 text-left"${add_attribute("aria-expanded", openFaq === i, 0)}><span class="font-bold text-lg text-secondary-dark">${escape(question)}</span> <span class="${"shrink-0 w-7 h-7 rounded-full border flex items-center justify-center transition " + escape(
      openFaq === i ? "rotate-45 bg-primary-dark text-white border-primary-dark" : "bg-off-white text-gray-600",
      true
    )}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M12 5v14M5 12h14"></path></svg> </span></button> ${openFaq === i ? `<p class="mt-3 text-gray-600 leading-relaxed">${escape(answer)}</p>` : ``} </div>`;
