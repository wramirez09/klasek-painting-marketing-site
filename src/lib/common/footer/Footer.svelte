<script lang="ts">
	...
	const usefulLinks: Link[] = [routes['contact'], routes['service-area'], routes['privacy']];
</script>

<footer class="flex flex-col items-center bg-secondary">
	<FooterCta />

	<div
		class="w-full container py-8 p-x grid lg:grid-cols-4 grid-cols-2 lg:gap-x-12 gap-y-8 xs:gap-x-4"
	>
		<!-- Logo & Social Links -->
		<div class="flex flex-col gap-6 lg:col-span-1 col-span-2 max-w-[300px] mx-auto">
			<a href="/">
				<enhanced:img src={logo} alt="Klasek Painting" class="w-full h-auto" loading="lazy" />
			</a>

			<FooterSocialLinks />
		</div>

		<div class="hidden lg:grid grid-cols-1 grid-rows-[150px_auto] gap-2">
			<Map lazy={true} />
		...
		<FooterLinksColumn title="Contact Us">
			<li class="mb-2 relative">
				<PhoneIcon class="size-4 absolute top-1/2 -translate-y-1/2 right-[calc(100%+6px)]" />
				<ClickToCall variant="link" />
			</li>

			<li class="relative">
				<LocationIcon class="size-5 absolute top-1 right-[calc(100%+6px)]" />
				<a class="underline underline-offset-2" href={googleMapsHref} target="_blank">
